...
# Check help flag
if [ "${1}" == "--help" ]; then
    [ ! -f ~/.magetools/src/docs/env:vars-edit.txt ] && error_message "No help documentation found" && exit 1
    (cat ~/.magetools/src/docs/env:vars-edit.txt | less -c) && exit 0
fi

# Check final args
[ ${#} -ne 1 ] && error_message "Syntax error when executing ${color_yellow}env:vars-edit${color_none} type ${color_yellow}mage env:vars-edit --help${color_none} to see the right syntax" && exit 1
